...
namespace NINA.MGEN {
    ...
    public class MGEN {
        ...
        public async Task<ICollection<FT_DEVICE_INFO_NODE>> Scan(CancellationToken ct = default(CancellationToken)) {
            try {
        ...
        public Task DetectAndOpen(CancellationToken ct = default(CancellationToken)) {
        ...
        public async Task Open(FT_DEVICE_INFO_NODE device, CancellationToken ct = default(CancellationToken)) {
            try {
                await lockObj.WaitAsync(ct);
        ...
        public Task<LEDState> ReadLEDState(CancellationToken ct = default(CancellationToken)) {
            var command = new GetLEDStatesCommand();
            return this.Send(command, ct);
        }

        public async Task<Bitmap> ReadDisplay(Color primaryColor, Color backgroundColor, CancellationToken ct = default(CancellationToken)) {
        ...
        public async Task<bool> IsBootMode(CancellationToken ct = default(CancellationToken)) {
            var command = new QueryDeviceCommand();
            var response = await this.Send(command, ct);
            return response.IsBootMode;
        }

        public async Task<bool> EnterNormalMode(CancellationToken ct = default(CancellationToken)) {
            var command = new EnterNormalModeCommand();
            var result = await this.Send(command, ct);
            return result.Success;
        }

        public async Task<string> GetFirmwareVersion(CancellationToken ct = default(CancellationToken)) {
            var command = new FirmwareVersionCommand();
            var response = await this.Send(command, ct);
            return response.Version;
        }

        public Task<ImagingParameter> GetImagingParameter(CancellationToken ct = default(CancellationToken)) {
            var command = new GetImagingParameterCommand();
            return this.Send(command, ct);
        }

        public Task<MGENResult> SetImagingParameter(byte gain, ushort exposureTime, byte threshold, CancellationToken ct = default(CancellationToken)) {
            var command = new SetImagingParameterCommand(gain, exposureTime, threshold);
            return this.Send(command, ct);
        }

        public async Task<bool> StartCamera(CancellationToken ct = default(CancellationToken)) {
        ...
        public async Task<bool> StopCamera(CancellationToken ct = default(CancellationToken)) {
            var command = new StopCameraCommand();
            var result = await this.Send(command, ct);
            return result.Success;
        }

        public async Task<bool> StartCalibration(CancellationToken ct = default(CancellationToken)) {
            var command = new StartCalibrationCommand();
            var result = await this.Send(command, ct);
            return result.Success;
        }

        public async Task<bool> CancelCalibration(CancellationToken ct = default(CancellationToken)) {
            var command = new CancelCalibrationCommand();
            var result = await this.Send(command, ct);
            return result.Success;
        }

        public async Task<bool> IsGuidingActive(CancellationToken ct = default(CancellationToken)) {
            var command = new QueryCommand(QueryCommand.QueryCommandFlag.AutoguidingState);
            var result = await this.Send(command, ct);
            return result.AutoGuiderActive;
        }

        public async Task<bool> StartGuiding(CancellationToken ct = default(CancellationToken)) {
            var command = new StartGuidingCommand();
            var result = await this.Send(command, ct);
            return result.Success;
        }

        public async Task<bool> StopGuiding(CancellationToken ct = default(CancellationToken)) {
            var command = new StopGuidingCommand();
            var result = await this.Send(command, ct);
            return result.Success;
        }

        public Task<GuidingResult> QueryGuideState(CancellationToken ct = default(CancellationToken)) {
            var command = new QueryCommand(QueryCommand.QueryCommandFlag.All);
            return this.Send(command, ct);
        }

        public async Task<bool> Dither(CancellationToken ct = default(CancellationToken)) {
        ...
        public async Task<CalibrationStatusResult> QueryCalibration(CancellationToken ct = default(CancellationToken)) {
            var command = new QueryCalibrationCommand();
            var response = await this.Send(command, ct);
            return response;
        }

        public async Task Disconnect(CancellationToken ct = default(CancellationToken)) {
            try {
                await lockObj.WaitAsync(ct);
        ...
        public Task<MGENResult> SetNewGuidingPosition(StarData starDetail, CancellationToken ct = default(CancellationToken)) {
            var command = new SetNewGuidingPositionCommand(starDetail);
            return this.Send(command, ct);
        }

        public Task<StarData> GetStarData(byte starIndex, CancellationToken ct = default(CancellationToken)) {
            var command = new GetStarDataCommand(starIndex);
            return this.Send(command, ct);
        }

        public Task<StarSearchResult> StartStarSearch(byte gain, ushort exposureTime, CancellationToken ct = default(CancellationToken)) {
            var command = new StartStarSearchCommand(gain, exposureTime);
            return this.Send(command, ct);
        }

        public Task<T> StartGenericCommand<T>(MGENCommand<T> command, CancellationToken ct = default(CancellationToken)) where T : IMGENResult {
            return this.Send(command, ct);
        }
