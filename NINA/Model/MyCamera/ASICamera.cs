...
namespace NINA.Model.MyCamera {
    ...
    public class ASICamera : BaseINPC, ICamera {
        ...
        private ASICameraDll.ASI_CAMERA_INFO Info {
            // [obsolete] info is cached only while camera is open[过时]信息仅在相机打开时缓存
            get
            {
                if (_info == null) {
                    // this needs to be called otherwise GetCameraProperties shuts down other instances of the camera这需要调用，否则GetCameraProperties会关闭相机的其他实例
                    ASICameraDll.OpenCamera(_cameraId);
                    // at this point we might as well cache the properties anyway无论如何，在这一点上，我们也可以缓存属性
        ...
        public double Temperature {
            get {
                return GetControlValue(ASICameraDll.ASI_CONTROL_TYPE.ASI_TEMPERATURE) / 10.0; //ASI driver gets temperature in Celsius * 10、ASI驱动器的温度单位为摄氏度*10
            }
        }
        ...
        public double TemperatureSetPoint {
            ...
            set {
                if (CanSetTemperature) {
                    //need to be an integer for ASI cameras 需要是ASI相机的整数
        ...
        public int BitDepth {
            get {
                //currently ASI camera values are stretched to fit 16 bit当前ASI相机值被拉伸以适合16位
                return 16;
            }
        }
        ...
        public void StartExposure(CaptureSequence sequence) {
            ...
            var isDarkFrame = sequence.ImageType == CaptureSequence.ImageTypes.DARK ||         //dark就是一个暗场，在黑暗环境下拍摄图片，记录噪声
                               sequence.ImageType == CaptureSequence.ImageTypes.BIAS ||        //偏置场，跟相机电路相关的
                               sequence.ImageType == CaptureSequence.ImageTypes.DARKFLAT;      //均匀度测试，不同像素的放大电路实际放大倍数存在差异，需要进行flat矫正。
                                                                                               //需要进一步在网上搜索学习天文摄影中的暗场，偏置场，flat场
            if (EnableSubSample) { 
                CaptureAreaInfo = new CaptureAreaInfo(
                    ...
                    new Size(SubSampleWidth / BinX - (SubSampleWidth / BinX % 8),
                             SubSampleHeight / BinY - (SubSampleHeight / BinY % 2) //设置子区域的中心点和尺寸。天文摄影中一幅图像很大，有时候我们只需要重点关注其中的一小部分区域，就必须有子采样区域
                    ),
                    BinX,
