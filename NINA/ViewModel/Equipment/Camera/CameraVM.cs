...
//这个文件是利用Inferce Icamera的基本函数，封装为几个高级别的函数。

namespace NINA.ViewModel.Equipment.Camera {

    internal class CameraVM : DockableVM, ICameraVM {

        public CameraVM(IProfileService profileService, ICameraMediator cameraMediator,  IApplicationStatusMediator applicationStatusMediator) : base(profileService) {
            Title = "LblCamera";
            ImageGeometry = (System.Windows.Media.GeometryGroup)System.Windows.Application.Current.Resources["CameraSVG"];

            CameraChooserVM = new CameraChooserVM(profileService);
        ...
        public Model.MyCamera.ICamera Cam {
            ...
            private set {
                _cam = value;
                RaisePropertyChanged();//从property setter调用 to notify the framework that an Entity member has changed.
            }
        }
        ...
        public async Task<IExposureData> Download(CancellationToken token) {
            ...
            if (CameraInfo.Connected == true) {
                ...
                var output = await Cam.DownloadExposure(token); //DownLoadExposure是DLL函数
                seqDuration.Stop();
